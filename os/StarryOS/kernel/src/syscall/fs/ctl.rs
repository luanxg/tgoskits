...
pub fn sys_fchdir(dirfd: i32) -> AxResult<isize> {
    ...
    current().as_thread().proc_data.fs_context.lock().set_current_dir(entry)?;
    Ok(0)
}
...
pub fn sys_getcwd(buf: *mut u8, size: isize) -> AxResult<isize> {
    ...
    let cwd = current().as_thread().proc_data.fs_context.lock().current_dir().absolute_path()?;
...
pub fn sys_sync() -> AxResult<isize> {
    ...
    current().as_thread().proc_data.fs_context.lock().root_dir().sync(false)?;
    Ok(0)
}
