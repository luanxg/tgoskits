...
fn do_execve(
    ...
    // ----------------------------------------------------------------
    // zap 是 Linux 内核术语，源自 zap_threads() / zap_other_threads()，意思是强制终止兄弟线程，类似于"击毙"。
    // 为什么 execve 需要 zap
    // execve 会用全新的地址空间替换当前进程的内存。但多线程进程中，其他线程还在旧地址空间上运行，直接替换地址空间会让它们崩溃。所以必须先停止它们：
    // 线程 A (调用 execve)          线程 B              线程 C
    //    │                         │                   │
    //    │ zap(B) ─────────────────→ 收到中断，退出     │
    //    │                         ↓ (do_exit)         │
    //    │ zap(C) ─────────────────────────────────────→ 退出
    ...
    //    ↓
    // 提交新地址空间
    // 这里的 zap_thread(tid) 就是向目标线程发送内部中断信号，让其从内核态/用户态协作退出：
    loop {
    ...
    // 描和关闭之间不会有新的 fd 出现。
    let fd_table_outer = current_fd_table();   // 获取当前进程的文件描述符表
    let mut fd_table = fd_table_outer.write();   // 加写锁
    let cloexec_fds: Vec<_> = fd_table
        .ids()                                                                      // 遍历所有 fd 编号
        .filter(|it| fd_table.get(*it).unwrap().cloexec)   // 筛选出 cloexec == true 的
        .collect();          // 收集到 Vec 中
    // 同一进程下的所有线程共享同一张 fd 表。 因为创建线程时使用了 CLONE_FILES。

    // cloexec 标志的含义
    // 当文件描述符设置了 cloexec（close-on-exec），它在 execve 时自动关闭，不会传递到新程序。对应的系统调用：
    // int fd = open("file.txt", O_CLOEXEC);           // 打开时直接设置
    // fcntl(fd, F_SETFD, FD_CLOEXEC);                 // 或打开后手动设置

    // 典型场景
    // execve("/bin/ls") 调用前：
    // fd 0: stdin    (cloexec=false) → execve 后保留
    // fd 1: stdout   (cloexec=false) → execve 后保留
    // fd 2: stderr   (cloexec=false) → execve 后保留
    // fd 3: temp.log (cloexec=true)  → execve 后关闭 ← 被收集到 cloexec_fds
    // fd 4: lock.pid (cloexec=true)  → execve 后关闭 ← 被收集到 cloexec_fds

    // execve只是替换了elf文件的内容，进程还是根据fork时候的Clone标志，选择继承哪些属性，这样理解对吧    
    // 不完全对。clone 标志决定 fork 时的行为，而 execve 有一套独立的保留/重置规则，两者互不干涉。
    // 正确的理解
    //  fork/clone           execve
    //  ┌──────────┐       ┌──────────────────────────┐
    //  │ 进程属性  │ ───→  │ 有些保留，有些被重置       │
    ...
    //execve 保留的属性（与 clone 标志无关）
    //  ┌─────────────────────────────┬───────────┐
    //  │            属性             │ execve 后 │
    //  ├─────────────────────────────┼───────────┤
    //  │ PID、PPID、进程组、session  │ 保留        │
    //  ├─────────────────────────────┼───────────┤
    //  │ 文件描述符（无 CLOEXEC）    │ 保留        │
    //  ├─────────────────────────────┼───────────┤
    //  │ 当前工作目录、根目录        │ 保留        │
    //  ├─────────────────────────────┼───────────┤
    //  │ 信号掩码（blocked signals） │ 保留        │
    //  ├─────────────────────────────┼───────────┤
    //  │ pending signals             │ 保留      │
    //  ├─────────────────────────────┼───────────┤
    //  │ nice 值、资源限制           │ 保留        │
    ...
    //  │ 栈                    │ 用新程序的新栈               │
    //  └───────────────────────┴──────────────────────────────┘
    // clone 标志只影响 fork 那一瞬间的事——父子进程共享什么、复制什么：
    // CLONE_VM → 共享地址空间（线程）
    // CLONE_FILES → 共享文件描述符表
    // CLONE_FS → 共享 cwd/root/umask
    // 但一旦 execve 发生，地址空间无论如何都会被替换——即使当初 fork 时用了 CLONE_VM，execve 后也不再共享。这就是为什么多线程程序中只有一个线程能执行 execve、其他线程必须被 zap 的原因。
